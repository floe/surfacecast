#!/bin/bash
./webrtc_client.py -d -n Aalborg -t butterbrot.org --fake -a "pulsesrc do-timestamp=true" \
	...
	-s "udpsrc do-timestamp=true port=3000 ! application/x-rtp,media=video,clock-rate=90000,encoding-name=H264,payload=96 ! rtpjitterbuffer ! rtph264depay ! avdec_h264 ! videoscale ! video/x-raw,width=1280,height=720 ! videoconvert" \
